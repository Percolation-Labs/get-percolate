...
# refusal until request_limit. The turn itself needs a model and is not run.
"$PY_BIN" - "$ROOT/ci/extract-runnable.py" "$ROOT/docs/src/agents.md" <<'EOF' | psql_ -f - >/dev/null \
...
found = [b for _, k, _, _, b in m.blocks_at(text) if k == "sql" and '"serves_agents": true' in b]
if len(found) != 1:
    sys.exit(f"agents.md has {len(found)} marked sql blocks registering a serves_agents server; expected 1")
print(found[0])
EOF
...
SYNC_BLOCK=$("$PY_BIN" "$ROOT/ci/extract-runnable.py" "$ROOT/docs/src/agents.md" --kind shell) \
    || fail "agents.md has no '<!-- run: shell -->' block that syncs the gateway's catalogue"
synced=""
for _ in $(seq 1 30); do
    synced=$(TOKEN="$TOKEN" bash -e -c "$SYNC_BLOCK" 2>&1) || true
    printf '%s' "$synced" | grep -q '"tools"' && break
    sleep 2
done
...
printf '%s' "$synced" | "$PY_BIN" -c '
import json, sys
bound = sys.argv[1].split()
try:
    tools = json.load(sys.stdin).get("tools") or []
except ValueError:
    tools = []
missing = [t for t in bound if t not in tools]
print(f"    catalogue: {tools}")
sys.exit(f"researcher is bound to {missing}, which the synced catalogue does not hold" if missing else 0)
' "$bound" || fail "agents.md's sync did not produce the tool researcher is bound to (runtime said: $synced)"
url=$(psql_ -tAc "select url from agentic.tool_servers where name = 'p8-agents'")
# Asked of the runtime's own settings and origin parser, inside its container,
# so the check agrees with the code that decides.
...
' "$url" || fail "the agent runtime would not send the caller's token to $url, so every delegation is refused"
